...
- **Chained tour triggered from the dashboard help button.** A `HelpCircle` icon (`id="tour-sidebar-help"`) next to the theme toggle in `documents.tsx` is the single entry point for replaying the tour. Clicking it clears BOTH `wa-tour-seen-documents` AND `wa-tour-seen-editor` localStorage flags, sets a `chainedTour` state, and remounts the dashboard `OnboardingTour`. When the dashboard tour completes (`onComplete`), if `chainedTour` is true and the user has at least one document, it navigates to `/editor/${docs[0].id}` — the editor tour then auto-shows (300ms delay) because `wa-tour-seen-editor` was cleared. There is **no help button in the editor**. New users get the dashboard tour on first dashboard visit and the editor tour on first editor visit automatically (both flags start unset). The dashboard tour has 7 steps (hero, new doc, world building, AI tools, stats, dark mode, help button); the editor tour has 13 steps (title, canvas, AI sidebar, AI token counter, word count goals, version history, world building, dark mode, undo/redo, export, upload images, tables, charts).
