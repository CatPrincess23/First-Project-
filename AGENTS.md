...
- **AI uses OpenRouter.** The `OPENROUTER_API_KEY` env var is read at startup. Model is `deepseek/deepseek-v4-flash` (free tier). Image generation is disabled (not supported via OpenRouter). The OpenAI SDK is configured with `baseURL: "https://openrouter.ai/api/v1"`.
...
- **Grammar check uses AI correction + local diff.** The `POST /api/ai/grammar` endpoint asks the model to return ONLY the corrected text (no JSON, no offset data — the model is unreliable at computing character positions). The backend then runs a local `wordDiff()` function that compares original vs corrected text token-by-token, computes 0-based character offsets, and returns an `errors[]` array with `message`, `suggestion`, `offset`, `length`, and `type` fields. This avoids fragile JSON parsing and inaccurate AI-computed positions.
- **Grammar errors are highlighted inline in the editor.** `editor.tsx` renders a `<div>` overlay behind the `<textarea>` with the same text content. Error spans get colored backgrounds (red=spelling, amber=grammar, blue=style) and bottom borders. The overlay scrolls in sync with the textarea. Clicking an error card in the sidebar calls `scrollToError()` which selects the error text and scrolls it into view. Highlights auto-clear when the user edits the content (offsets become stale).
