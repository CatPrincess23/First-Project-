...
- **`x-guest-id` header is the primary guest identity mechanism.** The frontend `guest-id.ts` generates a UUID on first visit, stores it in `localStorage`, and appends it as `x-guest-id` on every `/api/` request. The server reads this header in `resolveIdentity` (checked before cookie) and **also issues a signed `wa_guest` cookie** for the same UUID so identity survives JS crashes. The cookie is a fallback when `x-guest-id` is absent (crash scenario). No `POST /api/auth/guest` call is needed.
- **No Clerk auth required anywhere.** Local dev, preview deploys, and production all work without `CLERK_SECRET_KEY`. Identity is resolved from the signed `wa_guest` cookie first (server-HMAC verified), then the `x-guest-id` header (from localStorage), and finally a random UUID as last resort.
- **AI provider priority (top wins):** `GROQ_API_KEY` → `GROK_API_KEY` → `DEEPSEEK_API_KEY` → `GEMINI_API_KEY` → `OPENROUTER_API_KEY` (default). Groq routes to `https://api.groq.com/openai/v1` with model `llama-3.3-70b-versatile`. Grok/xAI routes to `https://api.x.ai/v1` with model `grok-2`. The OpenAI SDK is configured with `baseURL` set per-provider. 
