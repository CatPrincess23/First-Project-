...
- **AI token usage is tracked per user/day.** The `ai_usage` table (`lib/db/src/schema/usage.ts`) records `promptTokens`, `completionTokens`, `totalTokens`, and `requests` per `userId`+`date` (unique index). A `trackTokens(userId, usage)` helper in `ai.ts` is called after **every** `chat.completions.create()` call (all 10 endpoints: `/suggest`, `/grammar` scan+fix, `/scan-entities` by-name+bulk, `/image` prompt+SVG, `/summarize`, `/prologue`, `/chat`) and upserts the token counts via `onConflictDoUpdate` on the `(userId, date)` unique index. The `GET /api/ai/usage` endpoint (`ai.ts`) returns today's totals, a 30-day history, the daily limit (100K for Groq free tier), and the active provider/model name. The home dashboard (`documents.tsx`) shows today's token count with a progress bar against the daily limit instead of the old static "Unlimited" card. The Vite proxy must include `/api/ai` (it already does via the `/api` prefix) so the usage fetch resolves locally.
