...
- **`shouldRerenderOnTransaction: true` keeps the toolbar in sync.** TipTap v3's `useEditor` defaults to NOT re-rendering the component on transactions (for INP). Without re-renders, `editor.getAttributes("textStyle").fontFamily` and `editor.getAttributes("textStyle").fontSize` in the toolbar `<select>` dropdowns stay stale — after changing the font on selected text, the dropdown still shows the old font. Enabling `shouldRerenderOnTransaction: true` forces the component (including toolbar) to re-render on every transaction (keystroke, selection change, mark apply), keeping the font/size/color dropdowns and active-state toolbar buttons current. The `onUpdate` handler still uses `requestAnimationFrame` for `ed.getHTML()`, and `React.memo` still blocks parent-triggered re-renders, so the INP impact is limited to toolbar DOM diffing (~50 buttons).
- **Toolbar dropdowns save/restore editor selection.** The `<select>` font/size dropdowns and `<input type="color">` pickers steal focus from the editor when clicked, which can lose the text selection before the formatting command runs. `onMouseDown={e => e.preventDefault()}` on each control prevents focus from leaving the editor, so the text selection stays intact and `editor.chain().focus().setFontFamily(v).run()` applies to the correct range. A previous save/restore approach (`toolbarSelectionRef` + `runWithSelection()`) caused a one-step-behind race condition where the font change lagged one selection behind and was removed.
