...
- **`x-guest-id` header is the primary guest identity mechanism.** The frontend `guest-id.ts` generates a UUID on first visit, stores it in `localStorage`, and appends it as `x-guest-id` on every `/api/` request. The server reads this header in `resolveIdentity`. No cookie bootstrap or `POST /api/auth/guest` call is needed.
- **No Clerk auth required anywhere.** Local dev, preview deploys, and production all work without `CLERK_SECRET_KEY`. The dev-user injection in `app.ts` is a fallback that never actually runs in practice (identity is always resolved from `x-guest-id` or IP hash).
...
- **Word count strips HTML on both server and client.** The server's `countWords()` in `api-server/src/routes/documents.ts` strips HTML tags before counting (`text.replace(/<[^>]+>/g, " ")`), matching the client-side `stripHtml()` in `editor.tsx`. Word counts are **recalculated from content on every API request** rather than relying on potentially stale DB values, ensuring dashboard totals, per-document counts, and the editor always show the same number.
