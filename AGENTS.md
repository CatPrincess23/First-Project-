...
- **Toolbar dropdowns save/restore editor selection.** The `<select>` font/size dropdowns and `<input type="color">` pickers steal focus from the editor when clicked, which can lose the text selection before the formatting command runs. `ToolbarSelect` accepts an `onMouseDown` callback that saves `{from, to}` from `editor.state.selection` into `toolbarSelectionRef`. The `runWithSelection()` wrapper restores the selection via `editor.chain().focus().setTextSelection(...)` before running the font/size/color command, ensuring changes apply to the originally selected text.
